...
export const printApprovisionnementPDF = (data) => {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(data.benificiaire_nom + (data.fonction ? " (" + data.fonction + ")" : "") || 'N/A', leftCol + 35, yPos);

  doc.setFont('helvetica', 'bold');
  doc.text('Fonction :', rightCol, yPos);
  doc.setFont('helvetica', 'normal');
  doc.text(String(data.fonction || 'N/A'), rightCol + 35, yPos);
  
  yPos += lineHeight;
  
  // Row 2: Police | N° Civil
  doc.setFont('helvetica', 'bold');
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('N° Civil :', rightCol, yPos);
  doc.setFont('helvetica', 'normal');
  doc.text(String(data.ncivil || 'N/A'), rightCol + 35, yPos);
  
  yPos += lineHeight;
  
  // Row 3: Marque | Carburant
  doc.setFont('helvetica', 'bold');
  ...
  doc.setFont('helvetica', 'bold');
  doc.text('Carburant :', rightCol, yPos);
  doc.setFont('helvetica', 'normal');
  doc.text(String(data.carburant || 'gazoil'), rightCol + 35, yPos);

  yPos += lineHeight;
  
  // Row 4: Service | KM précédent
  doc.setFont('helvetica', 'bold');
  ...
  // Row 6: Quantité
  doc.setFont('helvetica', 'bold');
  doc.text('Quantité :', leftCol, yPos);
  doc.setFont('helvetica', 'normal');
  doc.text(String(data.qte || '0') + ' L', leftCol + 35, yPos);
  
  // Only show quota info for DOTATION type
  if (data.type_approvi === 'DOTATION') {
    if (data.qte_consomme !== undefined) {
      doc.setFont('helvetica', 'bold');
      doc.text('Qté Consommée :', rightCol, yPos);
      doc.setFont('helvetica', 'normal');
      doc.text(String(data.qte_consomme || '0') + ' L', rightCol + 35, yPos);
    }
    ...
    if (data.quota) {
      ...
      doc.setFont('helvetica', 'normal');
      doc.text(String(data.quota || '0') + ' L', leftCol + 35, yPos);
    }
    
    if (data.reste !== undefined) {
      doc.setFont('helvetica', 'bold');
      doc.text('Reste :', rightCol, yPos);
      doc.setFont('helvetica', 'normal');
      doc.text(String(data.reste || '0') + ' L', rightCol + 35, yPos);
    }
  }
...
export const printDotationApprovisionnement = (approData, dotationData) => {
  printApprovisionnementPDF({
    ...
    police: dotationData?.police || approData.police,
    ncivil: dotationData?.nCivil || approData.ncivil,
    marque: dotationData?.marque || approData.marque,
    carburant: dotationData?.carburant || approData.carburant,
    benificiaire_nom: dotationData?.benificiaire || approData.benificiaire_nom,
    fonction: dotationData?.fonction,
    service_nom: dotationData?.service || approData.service_nom,
    direction: dotationData?.direction || approData.direction,
    qte: approData.qte,
    km_precedent: approData.km_precedent,
    km: approData.km,
    quota: dotationData?.quota,
    qte_consomme: dotationData?.qte_consomme,
    reste: dotationData?.reste
  });
};
...
export const printMissionApprovisionnement = (data) => {
  printApprovisionnementPDF({
    ...
    ncivil: '', // Mission doesn't have vehicle details
    marque: '', // Mission doesn't have vehicle details
    carburant: 'gazoil',
