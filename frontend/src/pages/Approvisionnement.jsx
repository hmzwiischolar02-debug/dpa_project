...
export default function Approvisionnement() {
  ...
  const searchMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      setFormData({
        qte: '',
        km_actuel: data.km.toString(),  // ← FIXED
      });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      try {
        generateBonPDF(vehicleData, formData);
        toast.success('Bon PDF généré!');
      } catch (error) {
  ...
  const handleSubmit = (e) => {
    ...
    const data = {
      ...
      km_precedent: vehicleData.km,  // ← FIXED
      km: parseInt(formData.km_actuel),  // ← FIXED
    };
          ...
          {vehicleData && (
            <div className="space-y-6 animate-fade-in">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <InfoField label="KM" value={vehicleData.km.toLocaleString()} />
                ...
                <InfoField label="Qte" value={`${vehicleData.quota} L`} />
